...
def _refresh_dev_names():
    try:
        ...
        if len(_generated_names) and getattr(settings, 'MEDIA_REFRESH_ONCE', False):
...
def media_urls(key, refresh=False):
    if media_settings.MEDIA_DEV_MODE:
        if not hasattr(media_urls, 'names_generated') or refresh:
            _refresh_dev_names()
            media_urls.names_generated = True
...
try:
    _persistent_cache = caches['mediagenerator']
